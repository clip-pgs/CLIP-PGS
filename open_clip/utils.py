...
def unpatchify(x, p=16):
    """
    x: (N, L, patch_size**2 * 3) - input patches
    imgs: (N, 3, H, W) - output reconstructed images
    """
    N, L, patch_dim = x.shape
    assert patch_dim == p**2 * 3, "Patch dimensions don't match the expected size."
    
    # Calculate original image size
    h = w = int(L**0.5)
    assert h * w == L, "The number of patches must be a perfect square."
    
    # Reshape the patches back into (N, h, w, p, p, 3)
    x = x.reshape(N, h, w, p, p, 3)
    
    # Re-arrange to get (N, 3, H, W)
