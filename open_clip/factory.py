...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if target_mask_ratio is not None:
            # override the default target mask ratio
            model_cfg["vision_cfg"]["target_mask_ratio"] = target_mask_ratio

        if min_mask_ratio is not None:
            # override the default min mask ratio
            model_cfg["vision_cfg"]["min_mask_ratio"] = min_mask_ratio

        if use_ed is not None:
            # override the default use_ed
            model_cfg["vision_cfg"]["use_ed"] = use_ed

        if use_otn is not None:
            # override the default use_sinkhorn
            model_cfg["vision_cfg"]["use_otn"] = use_otn
